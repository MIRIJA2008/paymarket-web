  ...
  useEffect(() => {
    const initScanner = async () => {
      try {
        ...
        // Utiliser la cam̩ra arri̬re si disponible
          ...
          await codeReader.decodeFromVideoDevice(selectedDeviceId, videoRef.current, (result, err) => {
            if (result && scanning) {
              ...
              try {
                ...
                // V̩rifier que le QR code est valide pour PayMarket
                if (data.merchantId && data.merchantName) {
                  setScanning(false);
                  onScanSuccess(data);
                  toast.success(`Marchand trouv̩: ${data.merchantName}`);
                  stopScanner();
                } else {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center">
        <div className="bg-white rounded-xl p-6 max-w-md mx-4 text-center">
          ...
          <h3 className="text-xl font-bold mb-2">Cam̩ra non disponible</h3>
          <p className="text-gray-600 mb-4">{error || 'Veuillez v̩rifier que vous avez une cam̩ra et les permissions n̩cessaires'}</p>
          <button
            onClick={onClose}
